...
class Orders extends Indexfilters
{
	...
	{		
	   $join = [
		...
		while ($order = $orders[1]->fetch_assoc()) {
			$orderArray[] = self::arrayOrders($order);
		}
	...
	{	
		$orderElements = array(
			...
			'estado_es' => $order['estado_es'],
			'estado_en' => $order['estado_en'],
			'id_direccion_envio' => $order['id_direccion_envio'],
			'nombre_direccion' => $order['nombre_direccion'],
			'apellido_direccion' => $order['apellido_direccion'],
			'nombre_estado' => $order['nombre_estado'],
	...
	{		
		...
		while ($det = $detail[1]->fetch_assoc()) {
			...
			$ordersData[] = array(
				...
				'serie' => $det['serie'],
				'nombre_producto_es' => $det['nombre_producto_es'],
				'nombre_producto_en' => $det['nombre_producto_en'],
				'item_list' => $det['item_list'],
	...
	{	
		...
		$join = [
			...
			['LEFT','estados','estados.id_estado_eu = usuarios_direcciones.id_estado_eu'],
			['LEFT','ciudades','ciudades.id_ciudad = usuarios_direcciones.id_ciudad'],
			['INNER','venta_personalizar','venta_personalizar.serial_venta = venta_detalle.serial_venta'],
		];
