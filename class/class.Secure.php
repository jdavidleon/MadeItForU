...
{	
	...
	static public function retornoError($error)
	{	
		if (!isset(Login::$url)) {
			$url = URL_PAGE;
		}else{
		...
		$url_error = URL_PAGE.'/'.Login::$lang."/pages/login/error/".$error."/".$url;
		// echo $url_error;
		// exit();	
	?>	
	...
	static public function validar_correo($correo){
	...
	static public function verificarIntentos($correo){	
		return CRUD::find('usuarios_intentos','intentos','correo_usuario = ?',['s',$correo]);		 
	}
	...
	static public function intentosSesion($correo){
		...
		if ($consulta[1]->num_rows == 1) {
			...
			$actualizar = CRUD::update('usuarios_intentos',$set,'correo_usuario = ?',$params);
		}elseif ($consulta[1]->num_rows == 0) {
			$data = [
				'correo_usuario' => $correo,
				'intentos' => 1
			];
			$intentos_logueo = CRUD::insert('usuarios_intentos',$data);
		}
	}
	...
	static public function resetIntentos($correo){
		...
		if ($consult[1]->num_rows == 1) {
			$params = array("s",$correo);
			$eliminar = CRUD::delete('usuarios_intentos','correo_usuario = ?',$params);
		}
	}
