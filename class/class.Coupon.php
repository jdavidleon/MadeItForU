...
class Coupon extends Secure
{
	...
	{	
		...
		if (isset($_SESSION['id_usuario'])) {
			...
			$where = 'id_usuario = ? AND id_producto_cupon <> ? AND tm_delete IS NULL AND tm_used IS NULL';
			$params = ['ii',$_SESSION['id_usuario'],$cupon['id_producto_cupon']];
			...
			Cookie::createCookie('coupon',$set,60*60*24*8);
		}
		return true;
	}
